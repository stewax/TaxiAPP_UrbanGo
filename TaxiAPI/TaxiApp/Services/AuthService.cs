...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResult?> RegisterAsync(string fullName, string phone, string password)
    {
        try
        {
            // 🔥 Роль "client" передается на сервере автоматически
            var registerData = new { fullName, phone, password };
    ...
    public void SetToken(string token)
    {
        _token = token;
        SaveToken(token);
        System.Diagnostics.Debug.WriteLine($"✅ Токен сохранен: {token.Substring(0, Math.Min(20, token.Length))}...");
    }

    public string? GetToken()
    {
        System.Diagnostics.Debug.WriteLine($"🔑 GetToken: {(_token != null ? "токен есть" : "токена нет")}");
        return _token;
    }

    public void ClearToken()
    {
        System.Diagnostics.Debug.WriteLine("🗑 ClearToken: очистка токена...");
        _token = null;
        DeleteToken();
    }
    ...
    private void SaveToken(string token)
    {
        try
        {
            ...
            System.Diagnostics.Debug.WriteLine($"💾 Токен сохранен в: {tokenPath}");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Ошибка сохранения токена: {ex.Message}");
        }
    }
    ...
    private void LoadToken()
    {
        try
        {
            ...
            if (File.Exists(tokenPath))
            {
                ...
                System.Diagnostics.Debug.WriteLine($"📥 Токен загружен из файла: {_token.Substring(0, Math.Min(20, _token.Length))}...");
            }
            else
            {
                System.Diagnostics.Debug.WriteLine("📭 Файл токена не найден");
            }
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Ошибка загрузки токена: {ex.Message}");
        }
    }
    ...
    private void DeleteToken()
    {
        try
        {
            ...
            if (File.Exists(tokenPath))
            {
                ...
                System.Diagnostics.Debug.WriteLine($"🗑 Файл токена удален: {tokenPath}");
            }
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Ошибка удаления токена: {ex.Message}");
        }
    }
}
